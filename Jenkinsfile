...
def S390X_LOCAL_IMAGE = 'account:v0.3-s390x'
def AMD64_LOCAL_IMAGE = 'account:v0.3-amd64'
def TAG = 'v0.3'
...
def deleteOCPDeploymentSVCRouteIfExist() {
        ...
        try {
                sh "oc delete deployment account-app" 
                sh "oc delete svc account" 
                sh "oc delete route account" 
                        echo 'Succeeded!'
                      } catch (err) {
                         echo " the resource you are trying to delete doest not exist: account"
                       //echo "Failed: ${err}"
                     }
...
node('workers'){
 try {
   ...
   dir('target') {
                    sh "cp bank-account-1.0-SNAPSHOT.jar ../build"
    ...
    // sh "operator-sdk build ${S390X_LOCAL_IMAGE} --image-build-args='--arch=s390x'"
       ...
       stage('create manifest'){
        ...
        sh "podman manifest create account:${TAG}"
       }
       stage('add manifest list'){
       
         sh "podman manifest add account:${TAG} containers-storage:localhost/${AMD64_LOCAL_IMAGE}"
         sh "podman manifest add account:${TAG}  containers-storage:localhost/${S390X_LOCAL_IMAGE}"
      }
       stage('Push multi-arch image'){
        ...
        //sh "skopeo delete docker://${DOCKER_REGISTRY}/${DOCKER_REGISTRY_USER}/account:${TAG} "  //delete the image if it exists.
        sh "podman manifest push account:${TAG} docker://${DOCKER_REGISTRY}/${DOCKER_REGISTRY_USER}/account:${TAG}" 
    ...
    stage('Pre-integration Tests'){
            parallel(
                ...
                'Quality Tests': {
                      ...
                      //  sh "docker run --rm account:v0.3-amd64"
                    sh "podman run --publish-all --rm --name account -d account:v0.3-s390x"
         ...
         if(env.BRANCH_NAME == 'development' || env.BRANCH_NAME == 'preproduction'){
              ...
              dir('deploy') {
               ...
               createOCPNamespaceIfNotExist('staging-deployment')
                ...
                sh "oc create -f bank-account.yaml" // & deployment config
                sh "oc expose service account" // Push new stream
        ...
        if(env.BRANCH_NAME == 'master'){
           timeout(time: 2, unit: "HOURS") {
            ...
            dir('deploy') {
                ...
                deleteOCPDeploymentSVCRouteIfExist()
                sh "oc create -f bank-account-prod.yaml" // & deployment config
                sh "oc expose service account" // Push new stream
